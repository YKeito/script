"~/Nakano_RNAseq/network_analysis/script/CY151620ExP_CoExpNet/CY151620_CoExpMCL_GO_Analysis.R"
before <- proc.time()
#load package----
library(dplyr)
library(stringr)
...
TAIR10.GO <- read.table(file = "~/nishioka/PublishedData/ATH_GO_GOSLIM.txt", skip = 5, sep = "\t", quote = "", stringsAsFactors = F, fill = T)
...
#GO analysis----
N <- MasterTable %>% nrow()
#T.MCLNum <- MasterTable$MCLNum[MasterTable$MCLNum != 0] %>% unique()
T.MCLNum <- c(2, 5, 6, 21)
T.Aspect <- c("P", "C", "F")
allGO.Table <- c()
i <- 1
...
saveRDS(object = allGO.Table, file = "~/nishioka/RDS/GOTable.rds")
write.table(x = allGO.Table, file = "~/nishioka/Table/GOTable.txt", sep = "\t", quote = F, row.names = F)
after <- proc.time()
print(after - before)#8419.7 sec, 140.3 min, 2.34 hr