...
g <- g + coord_flip()
plot(g)
ggsave(filename = "~/IMO/Image/20201222DEGs_AtRegNet_GSEV.png", plot = g, width = 12, height = 8)
write.table(x = T.matrix, file = "~/IMO/Table/20201222DEGs_AtRegNet_GSEVScore.txt", sep = "\t", quote = F, row.names = F)
write.table(x = DEGs.AtRegNet.Table, 
            file = "~/IMO/Table/20201222DEGs_AtRegNet_GSEVTable.txt", sep = "\t", quote = F, row.names = F)